...
func Setup(db *sql.DB) http.Handler {
	...
	// Meetings — code route MUST be before {id} routes
	protected.HandleFunc("GET /api/meetings", meet.GetMeetings)
	protected.HandleFunc("POST /api/meetings", meet.CreateMeeting)
	protected.HandleFunc("GET /api/meetings/code/{code}", meet.GetMeetingByCode)
	protected.HandleFunc("DELETE /api/meetings/{id}", meet.DeleteMeeting)
	...
	// IMPORTANT: more specific paths must come before wildcards
	...
	mux.Handle("/api/meetings/code/", middleware.Auth(protected))
	mux.Handle("/api/meetings/", middleware.Auth(protected))
